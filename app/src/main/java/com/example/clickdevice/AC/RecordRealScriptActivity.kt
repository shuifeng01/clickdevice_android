...
class RecordRealScriptActivity : AppCompatActivity(), RecordScriptExecutor.RecordScriptInterface {
        ...
        binding?.btnComplete?.setOnClickListener {
            ...
            editName?.run {
                GlobalScope.launch(IOCoroutineContext()) {
                    viewModel?.saveScript(this@RecordRealScriptActivity, editName.toString())
                    MainScope().launch {
                        finish()
                        Toast.makeText(this@RecordRealScriptActivity, "保存成功", Toast.LENGTH_LONG).show()
                    }
                }
            }
            editName ?: run {
                Toast.makeText(this@RecordRealScriptActivity, "请输入脚本名", Toast.LENGTH_LONG).show()
            }
    ...
    private fun initSmallWindows() {
        ...
        smallWindowBinding = WindowRecordCanvesBinding.inflate(layoutInflater)
        smallWindowBinding?.recordClickView?.scriptListener =
    ...
    private fun notTouch() {
        smallWindowBinding?.recordClickView?.isEnabled = false
        ...
        smallWindowBinding?.recordClickView?.setBackgroundColor(0x30805000)
    }

    private fun canTouch() {
        smallWindowBinding?.recordClickView?.isEnabled = true
        val mLayoutParams = smallWindowsHelper?.mLayoutParams
        mLayoutParams?.flags = WindowManager.LayoutParams.FLAG_NOT_FOCUSABLE
        smallWindowsHelper?.mLayoutParams = mLayoutParams!!

        smallWindowBinding?.recordClickView?.setBackgroundColor(0x30005080)
    }
